...
class SettingsManager:
    ...
    def get_audio_delay(self, hdr_type, audio_format):
        """
        Get the appropriate audio delay based on the combination of HDR type
        and audio codec. For example, for Dolby Vision and TrueHD, retrieve
        the setting 'dolbyvision_truehd'.
        """
        if hdr_type is None or audio_format is None:
            xbmc.log(f"AOM_SettingsManager: Invalid hdr_type ({hdr_type}) or "
                     f"audio_format ({audio_format}). Returning 0.",
                     xbmc.LOGWARNING)
            return 0
    ...
    def store_audio_delay(self, setting_id, delay_ms):
        ...
        try:
            xbmc.log(f"AOM_SettingsManager: Storing delay for {setting_id}: "
                     f"{delay_ms} ms", xbmc.LOGDEBUG)
            self.settings.setInt(setting_id, delay_ms)
        except:
            xbmc.log(f"AOM_SettingsManager: Error storing delay for "
                     f"'{setting_id}'.", xbmc.LOGWARNING)

    def store_platform_hdr_full(self, platform_hdr_full):
        """Store the platform HDR full setting."""
        try:
            xbmc.log(f"AOM_SettingsManager: Storing platform HDR full: "
                     f"{platform_hdr_full}", xbmc.LOGDEBUG)
            self.settings.setBool("platform_hdr_full", platform_hdr_full)
        except:
            xbmc.log("AOM_SettingsManager: Error storing platform HDR full "
                     "setting.", xbmc.LOGWARNING)

    def store_advanced_hlg(self, advanced_hlg):
        """Store the advanced HLG setting."""
        try:
            xbmc.log(f"AOM_SettingsManager: Storing advanced HLG: "
                     f"{advanced_hlg}", xbmc.LOGDEBUG)
            self.settings.setBool("advanced_hlg", advanced_hlg)
        except:
            xbmc.log("AOM_SettingsManager: Error storing advanced HLG setting.",
                     xbmc.LOGWARNING)

    def store_new_install(self, new_install):
        """Store the new install setting."""
        try:
            xbmc.log(f"AOM_SettingsManager: Storing new install: {new_install}",
                     xbmc.LOGDEBUG)
            self.settings.setBool("new_install", new_install)
        except:
            xbmc.log("AOM_SettingsManager: Error storing new install setting.",
                     xbmc.LOGWARNING)
    ...
    def is_hdr_enabled(self, hdr_type):
        ...
        """
        if hdr_type is None:
            xbmc.log("AOM_SettingsManager: Invalid hdr_type (None). Returning "
                     "False.", xbmc.LOGWARNING)
            return False
